...
// Sort pending IO request by delay time
static int
PendingIOCompare(IORequest *x, IORequest *y)
{
    if (x->getCompleteTime() < y->getCompleteTime())
    {
        return -1;
    }
    else if (x->getCompleteTime() > y->getCompleteTime())
    {
        return 1;
    }
    else
...
void Kernel::Initialize()
{
    ...
    ioalarm = new IOAlarm();
    pendingIO = new SortedList<IORequest *>(PendingIOCompare);
